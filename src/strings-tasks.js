...
function countVowels(str) {
  ...
  for (let i = 0; i < target.length; i += 1) {
    let pos = lowCase.indexOf(target[i]);

    while (pos !== -1) {
      sum += 1;
      pos = lowCase.indexOf(target[i], pos + 1);
    }
  }
...
function isPalindrome(str) {
  let low = str.toLowerCase().replaceAll(' ', '');

  low = low.replaceAll('?', '').replaceAll('!', '').replaceAll(',', '');

  for (let i = 0; i < Math.floor(low.length / 2); i += 1) {
...
function invertCase(str) {
  let stroka = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str.charAt(i).codePointAt() > 90) {
      stroka += str.charAt(i).toUpperCase();
    } else stroka += str.charAt(i).toLowerCase();
  }
  return stroka;
}
